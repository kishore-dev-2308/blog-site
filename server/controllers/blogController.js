...
export const publishBlog = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Blog published successfully", blog: updatedBlog });
  } catch (error) {
    logger.error(error);
    res.status(500).json({ message: "Something went wrong" });
  }
...
export const unPublishBlog = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Blog unpublished successfully", blog: updatedBlog });
  } catch (error) {
    logger.error(error);
    res.status(500).json({ message: "Something went wrong" });
  }
...
export const makeFeaturedBlog = async (req, res) => {
  try {
    const blogId = req.params.id;
    const { featured } = req.body;

    await prisma.blog.updateMany({
      where: { id: { not: blogId }, isFeatured: true },
